...
LST2FAN         TC      VBZERO          # VB40 ZERO (USED WITH NOUN 20 OR 40 ONLY)
                ...
                TC      HELLORLD	# VB63 CUSTOM HELLORLD DISPLAY
                TCF     ALM/END
                TCF     ALM/END
                TCF     +1
                TCF     ALM/END
...
HELLORLD	TC	TESTXACT	# MAKE SURE ANOTHER EXTENDED VERB ISN'T RUNNING
		CAF	HWORDCOUNT	# LOAD WORD INDEX INTO A
		TS	HELLOOUTLP	# STORE INTO INDEX COUNTER
		CAF	HLOOPCOUNT	# LOAD CONSTANT 3
		TS	HELLOINMAX	# STORE FOR SUBTRACTION LATER

HELLOSTART	CAF	ZERO		# WE WILL LOOP 3 TIMES PER "PRO" ON DSKY
		TS	HELLOINLP	# ZERO INNER LOOP COUNTER

HELLOCOPY	INDEX	HELLOOUTLP	# INDEX SOURCE ADDRESS
		CAF	HELLOWORDS	# LOAD WORD INTO A
		INDEX 	HELLOINLP	# INDEX DESTINATION ADDRESS
		TS	FAILREG		# STORE WORD IN ERASABLE
		EXTEND
		AUG	HELLOINLP	# INCREMENT INNER LOOP
		CCS	HELLOOUTLP	# CHECK OUTER LOOP COUNT
		TCF	HELLOCONT	# IF > +0 CONTINUE COPYING
		TCF	HELLOTERM	# IF = +0 END

HELLOCONT	TS	HELLOOUTLP	# STORE DECREMENTED OUTER LOOP BACK
		CA	HELLOINLP	# READ INNER LOOP COUNT INTO A
		EXTEND
		SU	HELLOINMAX	# CHECK IF WE'VE COPIED 3 WORDS
		EXTEND
		BZF	HLOOPDONE	# IF SO, DISPLAY THEM ON THE DSKY AND WAIT FOR PRO
		TCF	HELLOCOPY	# OTHERWISE CONTINUE COPYING

HLOOPDONE	CAF	HELLOVN		# LOAD THE VERB/NOUN PAIR
		TC	NVSBWAIT	# EXECUTE IT ON THE DSKY IF NOT BUSY
		TC	ENDIDLE		# WAIT FOR PROCEED KEY
		TC	HELLOTERM	# IF TERMINATE
		TC	HELLOPRO	# IF PROCEED
		TC	HLOOPDONE	# IF ENTER

HELLOTERM	CAF	HELLOVN		# LOAD THE VERB/NOUN PAIR
		TC	NVSBWAIT	# EXECUTE IT ON THE DSKY IF NOT BUSY
		TC	FREEDSP		# FREE THE DISPLAY
		TC	ENDEXTVB	# END THE VERB

HELLOPRO	TC	HELLOSTART	# COPY THE NEXT 3 WORDS AND DISPLAY


HELLOVN		OCT	0531		# WE HIJACK THE SELF-TEST/ALARM ERASABLES
HLOOPCOUNT	DEC	3		# 3 WORDS PER DSKY DISPLAY
HWORDCOUNT	DEC	8		# 9 ASCII VALUES TAKING UP ONE WORD EACH
#	WE STORE THE WORDS IN REVERSE ORDER SO WE CAN LOAD THEM BY DECREMENTING AN INDEX
HELLOWORDS	OCT	41		# !
		OCT	104		# D
		OCT	114		# L
		OCT	122		# R
