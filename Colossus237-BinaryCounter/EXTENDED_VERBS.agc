...
## Page 230
		...
		TC	VERB98		# VB98 TEST BY JAMES PELSTER
...
VERB98		TC	TESTXACT	# MAKE SURE WE'RE THE ONLY EXTENDED VERB RUNNING
		TC	POSTJUMP	# JUMP TO OUR CODE PROPER IN BANK 5
		CADR	VERB98B


		BANK 5			# SHIFT OUR CODE INTO BANK 5 BECAUSE IT HAS MORE ROOM.
		BANK			# RELOCATE TO THE FIRST AVAILABLE BLOCK OF SPACE IN THE BANK.
VERB98B		CAF	ZERO		# LOAD THE FIRST 5 DIGITS INTO A
		TS	DSPTEM1		# STORE INTO DSPTEM1
		CAF	ZERO		# LOAD THE SECOND 5 DIGITS INTO A
		TS	DSPTEM1 + 1	# STORE INTO DSPTEM1 + 1
		CAF	ZERO		# LOAD THE THIRD 5 DIGITS INTO A
		TS	DSPTEM1 + 2	# STORE INTO DSPTEM1 + 2
		CAF	V98WAIT		# LOAD THE TASK DELAY (1 second)
		TC	TWIDDLE		# ACTIVATE THE WAITLIST TASK
		ADRES	V98TASK		# ADDRESS OF THE TASK
		CAF	V98DSP		# LOAD V/N PAIR
		TC	BANKCALL
		CADR	GOXDSPF		# CALL THE DSKY DISPLAY ROUTINE (EXTENDED VERB FLASHING)
		TC	V98DONE
		TC	V98DONE
		TC	V98DONE
V98DONE		TC	ENDEXT		# SIGNAL THAT OUR EXTENDED VERB IS DONE
		TC	ENDOFJOB	# EXIT OUT FULLY


V98DSP		VN	1625		# VERB 16, NOUN 25 CONSTANTS
					# FOR USE WITH DISPLAY INTERFACE ROUTINE
V98DATA1	DEC	01234		# DATA TO STORE IN N25 R1 (DSPTEM1)
V98DATA2	DEC	56789		# DATA TO STORE IN N25 R2 (DSPTEM1 + 1)
V98DATA3	DEC	01337		# DATA TO STORE IN N25 R3 (DSPTEM1 + 2)
V98INCR		DEC	10		# CONSTANT FOR INCREMENTING THE COUNTERS


V98TASK		CA	DSPTEM1 + 2	# START WITH (DSPTEM1 + 2)
		AD	V98INCR		# INCREMENT THE VALUE
		TS	DSPTEM1 + 2	# TRY TO STORE IN N25 R3
		TC	V98DONE2	# IF SUCCESSFUL, WE'RE DONE
		AD	DSPTEM1 + 1	# IF OVERFLOW, ADD OVERFLOW BIT PLUS R2
		TS	DSPTEM1 + 1	# TRY TO STORE IN N25 R2
		TC	V98DONE2	# IF SUCCESSFUL, WE'RE DONE
		AD	DSPTEM1		# IF OVERFLOW, ADD OVERFLOW BIT PLUS R1
		TS	DSPTEM1		# TRY TO STORE IN N25 R1
		NOOP			# IF OVERFLOW, ZERO THINGS OUT (TO BE PROGRAMMED!)
V98DONE2	TC	FIXDELAY	# RATHER THAN ENDING ENTIRELY, DELAY FOR 1 SECOND.
V98WAIT		DEC	100		# DELAY FOR INCREMENT TASK IN CENTISECONDS
		TC	V98TASK		# AT THE START OF THIS NEXT RUN, WE WILL BE HERE.
					# JUMP BACK UP TO THE START OF THE TASK CODE.
